 ...
 */
export async function generarNotificacionesDeVencimiento() {
  ...
  const cacheNotificaciones = new Set(
    notificacionesExistentes.map(n => `${n.trabajadorId}-${n.tareaId}-${n.mensaje}`)
  );

  const notificacionesACrear = [];

  for (const tarea of tareasActivas) {
    if (!tarea.trabajadorId) continue;

    const diasParaVencer = differenceInDays(tarea.fechaProgramada, hoy);
    const nombreTarea = tarea.tareaPlantilla.nombre;

    // Formatear fecha de vencimiento como: 13 de diciembre de 2025
    const fechaFormateada = format(tarea.fechaProgramada, "d 'de' MMMM yyyy", { locale: es });
