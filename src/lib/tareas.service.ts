...
import { Prisma,EstadoTarea, PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

type TareaFrontEstado = "pendiente" | "completado" | "atrasado";

const mapEstado = (estado: EstadoTarea): TareaFrontEstado => {
  if (estado === "COMPLETADA") return "completado";
  if (estado === "VENCIDA") return "atrasado";
  // PENDIENTE o EN_PROCESO
  return "pendiente";
};

export async function obtenerTareasPorTrabajador(
  opts?: { soloPendientes?: boolean; trabajadorId?: number }
) {
  const { soloPendientes = false, trabajadorId } = opts || {};

  const whereTrabajador: any = { status: true };
  if (trabajadorId) whereTrabajador.id_trabajador = trabajadorId;
