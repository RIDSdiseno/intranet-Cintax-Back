...
if (!SERVICE_ACCOUNT_EMAIL || !SERVICE_ACCOUNT_KEY) {
    throw new Error("Faltan GOOGLE_CLIENT_EMAIL o GOOGLE_PRIVATE_KEY para el service account");
}
...
function getDelegatedGmail(userEmail) {
    if (!userEmail.endsWith(`@${ALLOWED_DOMAIN}`)) {
        throw new Error(`Solo se pueden enviar correos como usuarios del dominio ${ALLOWED_DOMAIN}`);
    }
...
async function sendEmailAsUser({ fromUserEmail, to, subject, bodyText, attachments = [], }) {
    const gmail = getDelegatedGmail(fromUserEmail);
    console.log("[Correo] Enviando como:", fromUserEmail);
    console.log("[Correo] Adjuntos:", attachments.map((a) => ({
        filename: a.filename,
        mimeType: a.mimeType,
    ...
    for (const att of attachments) {
        ...
        lines.push("", `--${boundary}`, `Content-Type: ${att.mimeType}; name="${att.filename}"`, "Content-Transfer-Encoding: base64", `Content-Disposition: attachment; filename="${att.filename}"`, "", base64Content);
    }
