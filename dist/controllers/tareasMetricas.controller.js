...
const getMetricasSupervision = async (req, res) => {
    try {
        ...
        const trabajadorIds = Array.from(new Set(tareas
        ...
        for (const t of tareas) {
            ...
            if (!datosTrabajador)
                continue; // puede que el filtro de área lo haya excluido
        ...
        const totalCompletadas = agentes.reduce((acc, a) => acc + a.completadas, 0);
        const promedioTareasPorAgente = totalTareas > 0 && agentes.length > 0
        ...
        const porcentajeCompletadas = totalTareas > 0
            ...
            : 0;
        const tasaVencidas = totalTareas > 0 ? Math.round((totalVencidas / totalTareas) * 100) : 0;
        const tasaEnProceso = totalTareas > 0 ? Math.round((totalEnProceso / totalTareas) * 100) : 0;
        // =======================
        // KPIs POR AGENTE
        // =======================
        const agentesConKpi = agentes.map((a) => {
            const total = a.total;
            const porcentajeCompletadasAgente = total > 0 ? Math.round((a.completadas / total) * 100) : 0;
            const porcentajeVencidasAgente = total > 0 ? Math.round((a.vencidas / total) * 100) : 0;
            const porcentajePendientesAgente = total > 0 ? Math.round((a.pendientes / total) * 100) : 0;
            return {
                ...a,
        ...
        const mejorCumplimiento = agentesConKpi.length > 0
            ? [...agentesConKpi].sort((a, b) => b.porcentajeCompletadas - a.porcentajeCompletadas)[0]
            : null;
        const masVencidas = agentesConKpi.length > 0
            ? [...agentesConKpi].sort((a, b) => b.porcentajeVencidas - a.porcentajeVencidas)[0]
            : null;
        return res.json({
            filtros: {
                areaInterna,
...
const getMetricasAgente = async (req, res) => {
    try {
        ...
        for (const t of tareas) {
            ...
            const empresa = t.tareaPlantilla?.area ||
                t.rutCliente ||
        ...
        const pendientesPorTarea = Array.from(mapTarea.values()).sort((a, b) => b.cantidad - a.cantidad);
