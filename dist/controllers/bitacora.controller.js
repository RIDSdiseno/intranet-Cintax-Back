...
exports.upsertBitacora = upsertBitacora;
exports.listMisBitacoras = listMisBitacoras;
exports.listBitacoras = listBitacoras;
exports.getBitacoraById = getBitacoraById;
exports.updateBitacoraById = updateBitacoraById;
exports.deleteBitacoraById = deleteBitacoraById;
 ...
 * - Si es texto: agrega \n\n[HH:mm]\n...
 */
function appendWithTime(existing, incoming) {
 ...
 */
async function upsertBitacora(req, res, next) {
    try {
        ...
        const safeTitulo = titulo === null ? null : typeof titulo === "string" ? (titulo.trim() || null) : null;
        ...
        const nextContenido = writeMode === "replace" ? incoming : appendWithTime(existing.contenido || "", incoming);
 ...
 */
async function listMisBitacoras(req, res, next) {
    try {
        const user = mustUser(req);
        ...
        const where = { trabajadorId: user.id };
        if (desde || hasta) {
            where.fecha = {};
            if (desde)
 ...
 * (ADMIN/SUPERVISOR)
 */
async function listBitacoras(req, res, next) {
    try {
        const user = mustUser(req);
        if (!isPrivileged(user)) {
        ...
        const desde = typeof req.query.desde === "string" ? parseDay(req.query.desde) : undefined;
        const hasta = typeof req.query.hasta === "string" ? parseDay(req.query.hasta) : undefined;
        const trabajadorId = typeof req.query.trabajadorId === "string" && req.query.trabajadorId.trim()
 ...
 */
async function getBitacoraById(req, res, next) {
    try {
        const user = mustUser(req);
        ...
        if (!row)
            return fail(res, 404, "Bitácora no encontrada.");
 ...
 * - mode: append/replace
 */
async function updateBitacoraById(req, res, next) {
    try {
        const user = mustUser(req);
        const id = Number(req.params.id);
        if (Number.isNaN(id))
 ...
 */
async function deleteBitacoraById(req, res, next) {
    try {
        const user = mustUser(req);
        if (!isPrivileged(user)) {
            return fail(res, 403, "No autorizado. Solo administradores o supervisores pueden eliminar bitácoras.");
        }
        const id = Number(req.params.id);
        if (Number.isNaN(id))
            return fail(res, 400, "id inválido.");
